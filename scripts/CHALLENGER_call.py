...
parser.add_argument("-bs", "--batch-size", default=32, type=int, help="Mini-batch size for evaluation or inference.")
...
# Utils
parser.add_argument("-w", "--weight", default="outputs", type=str, metavar="PATH", help="Path to model weights.")
parser.add_argument("-o", "--output-dir", default="outputs", type=str, metavar="PATH", help="Directory for saving calls.")
...
OUTPUT_FILE =  os.path.join(args.output_dir, args.run_name + ".csv")
...
class CustomTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.SEG_tp_tn_fp_fn = 0
        self.SEG_tp_nocall = 0
        self.SEG_tp_duplication = 0
        self.SEG_tp_plus_fp_nocall = 0
        self.SEG_tp_plus_fp_duplication = 0 
        self.SEG_tp_plus_fp_deletion = 0
        self.SEG_tp_plus_fn_nocall = 0
        self.SEG_tp_plus_fn_duplication = 0
        self.SEG_tp_plus_fn_deletion = 0
        self.SEG_tp_deletion = 0
        ...
        self.latest_seg_loss = 0
        self.latest_cls_loss = 0
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=-1):
        ...
        ####### Segmentation Loss ####################
        seg_ce_loss_val = loss_fct(seg_logits[positive_flag].view(-1, 3), masked_labels.view(-1))
        dup_dice_loss_val = dice_loss((masked_predicted_seg==1).to(int), (masked_labels==1).to(int))
        del_dice_loss_val = dice_loss((masked_predicted_seg==2).to(int), (masked_labels==2).to(int))
        dice_loss_val = ((dup_dice_loss_val+del_dice_loss_val)/2)
        seg_loss = (seg_ce_loss_val) + (dice_loss_val)

        ####### Total Loss ##############
        loss = (5*seg_loss) + (2*cls_ce_loss_val)

        ####### Compute SEG Scores ########
        self.SEG_tp_nocall += (torch.logical_and(masked_predicted_seg == masked_labels,masked_predicted_seg == 0)).sum().item() 
        self.SEG_tp_duplication += (torch.logical_and(masked_predicted_seg == masked_labels,masked_predicted_seg == 1)).sum().item() 
        self.SEG_tp_deletion += (torch.logical_and(masked_predicted_seg == masked_labels,masked_predicted_seg == 2)).sum().item()

        self.SEG_tp_plus_fp_nocall += (masked_predicted_seg == 0).sum().item()
        self.SEG_tp_plus_fp_duplication += (masked_predicted_seg == 1).sum().item()
        self.SEG_tp_plus_fp_deletion += (masked_predicted_seg == 2).sum().item()

        self.SEG_tp_plus_fn_nocall += (masked_labels == 0).sum().item()
        self.SEG_tp_plus_fn_duplication += (masked_labels == 1).sum().item()
        self.SEG_tp_plus_fn_deletion += (masked_labels == 2).sum().item()
        self.SEG_tp_tn_fp_fn += masked_labels.size(0)    
...
def data_loader(tokenizer):
    ...
    def preprocess_function(examples):
        ...
        model_inputs["label_ids"] = tokenizer.encode("[CLS]"+"".join(["[NOCALL]"]*1000)+"[SEP]")
...
def set_trainer(roberta_model, dataset):
    ...
    def compute_metrics(eval_pred):
        CLS_nocall_prec, CLS_dup_prec, CLS_del_prec, CLS_nocall_recall, CLS_dup_recall, CLS_del_recall, CLS_nocall_f1, CLS_dup_f1, CLS_del_f1, CLS_overall_prec, CLS_overall_recall, CLS_overall_f1 = calculate_metrics(trainer.CLS_tp_nocall, trainer.CLS_tp_plus_fp_nocall, trainer.CLS_tp_duplication, trainer.CLS_tp_plus_fp_duplication, trainer.CLS_tp_deletion, trainer.CLS_tp_plus_fp_deletion, trainer.CLS_tp_plus_fn_nocall, trainer.CLS_tp_plus_fn_duplication, trainer.CLS_tp_plus_fn_deletion)
        cls_loss = torch.clone(trainer.latest_cls_loss)
        ...
        all_perfs = {
                        "CLS/NOCALL_f1":round(CLS_nocall_f1,3) , "CLS/NOCALL_prec" : round(CLS_nocall_prec,3) , "CLS/NOCALL_recall": round(CLS_nocall_recall,3) ,
                        "CLS/DUP_f1":round(CLS_dup_f1,3) , "CLS/DUP_prec" : round(CLS_dup_prec,3) , "CLS/DUP_recall": round(CLS_dup_recall,3) ,
                        "CLS/DEL_f1":round(CLS_del_f1,3) , "CLS/DEL_prec" : round(CLS_del_prec,3) , "CLS/DEL_recall": round(CLS_del_recall,3) ,
                        "CLS/Overall_f1":round(CLS_overall_f1,3) , "CLS/Overall_prec" : round(CLS_overall_prec,3) , "CLS/Overall_recall": round(CLS_overall_recall,3) ,
                        "CLS/Loss":round(cls_loss.item(),3)
                    }
...
if __name__ == "__main__":
    ...
    print(sum(p.numel() for p in roberta_model.parameters()))
    indices = np.array_split(np.arange(dataset.shape[0]), 1)
    for i, batch_ind in tqdm(enumerate(indices)):
        subset_dataset = dataset.select(batch_ind)
        output_df = subset_dataset.select_columns(["sample_name","chr","gene_name"]).to_pandas()
        predictions = trainer.predict(subset_dataset)
        _ , predicted_cls = torch.max(torch.Tensor(predictions.predictions[1]), 1)
        output_df["pred"] = predicted_cls
        output_df["pred"] = output_df["pred"].map({0: "<NO-CALL>", 1: "<DUP>",2:"<DEL>"})
        if(i==0):
            output_df.to_csv(OUTPUT_FILE, index=False, mode="w", header=True, sep="\t")
        else:
            output_df.to_csv(OUTPUT_FILE, index=False, mode="a", header=False, sep="\t")

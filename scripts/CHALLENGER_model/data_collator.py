...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    cnv_mlm_probability : float = 0.15 # MY_UPDATE
    cnv_mask_replace_prob: float = 0.8 # MY_UPDATE
    cnv_random_replace_prob: float = 0.1 # MY_UPDATE
    pad_to_multiple_of: Optional[int] = None
    ...
    def __post_init__(self):
        ...
        self.cnv_mask_id = self.tokenizer.encode("[CNV_MASK]")[0]# MY_UPDATE
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if self.mlm:
            ...
            batch["label_ids"],batch["labels"] = self.torch_mask_CNV_tokens(
                batch["label_ids"], special_tokens_mask=special_tokens_mask
            ) # MY_UPDATE
            
            #self.tokenizer.decode(batch["label_ids"][0])
    ...
    def torch_mask_CNV_tokens(self, inputs: Any, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
        import torch
        ...
        labels = inputs.clone()
        # We sample a few tokens in each sequence for MLM training (with probability `self.mlm_probability`)
        probability_matrix = torch.full(labels.shape, self.cnv_mlm_probability)
        if special_tokens_mask is None:
            special_tokens_mask = [
                self.tokenizer.get_special_tokens_mask(val, already_has_special_tokens=True) for val in labels.tolist()
            ]
        ...
        # cnv_mask_replace_prob% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = torch.bernoulli(torch.full(labels.shape, self.cnv_mask_replace_prob)).bool() & masked_indices
        inputs[indices_replaced] = self.cnv_mask_id # MY_UPDATE
        
        if self.cnv_mask_replace_prob == 1 or self.cnv_random_replace_prob == 0:
            return inputs, labels

        remaining_prob = 1 - self.cnv_mask_replace_prob
        # scaling the cnv_random_replace_prob to the remaining probability for example if
        # cnv_mask_replace_prob = 0.8 and cnv_random_replace_prob = 0.1,
        # then cnv_random_replace_prob = 0.1 / 0.2 = 0.5
        cnv_random_replace_prob_scaled = self.cnv_random_replace_prob / remaining_prob
        # random_replace_prob% of the time, we replace masked input tokens with random word
        indices_random = (
            torch.bernoulli(torch.full(labels.shape, cnv_random_replace_prob_scaled)).bool()
            & masked_indices
            & ~indices_replaced
        )
        random_words = torch.randint(len(self.tokenizer), labels.shape, dtype=torch.long)
        inputs[indices_random] = random_words[indices_random]
        # The rest of the time ((1-cnv_random_replace_prob-cnv_mask_replace_prob)% of the time) we keep the masked input tokens unchanged
        return inputs, labels
